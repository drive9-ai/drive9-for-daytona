...
async function sceneOutro(): Promise<void> {
  ...
  printInstant(`  ${bold}Three sandboxes. One filesystem. Complete isolation.${reset}`)
  printInstant('')
  printInstant(`  ${dim}1.${reset} Sandbox A: cloned repo + LLM analysis + forked workspace`)
  printInstant(`  ${dim}2.${reset} Sandbox B: ran tests on isolated fork`)
  printInstant(`  ${dim}3.${reset} Sandbox C: continued development on original (parallel)`)
  printInstant('')
  printInstant(`  ${dim}All sandboxes destroyed. All artifacts persist on drive9.${reset}`)
  printInstant('')
